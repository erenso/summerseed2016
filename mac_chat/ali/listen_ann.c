...
static const char *bssid_announcement = "\x00" "chat0";
static const char *bssid_data = "\x00" "chat1";
static const char *smac = "\x18\xcf\x5e\xde\x16\xc0";
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		/* filter mac */
		if (memcmp(pkt + 4, bssid_announcement, 6))
			continue;
		...
		FILE *people = fopen("people.txt", "a");
		fprintf(people, "%20s %02x:%02x:%02x:%02x:%02x:%02x\n", payload, pkt[10], pkt[11], pkt[12], pkt[13], pkt[14], pkt[15]);
		fclose(people);
	}
